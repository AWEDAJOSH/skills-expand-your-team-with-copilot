document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentDifficulty = null; // null means no filter, "" means show only unspecified
  ...
  function displayFilteredActivities() {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty !== null) {
        // If currentDifficulty is empty string, only show activities without difficulty
        if (currentDifficulty === "" && details.difficulty) {
          return;
        }
        // Otherwise, only show activities with matching difficulty
  ...
  function shareActivity(platform, name, details) {
    ...
    switch(platform) {
      ...
      case 'email':
        ...
        shareUrl = `mailto:?subject=${subject}&body=${encodedText}%20${encodedUrl}`;
        break;
    }
